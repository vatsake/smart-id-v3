...
class Certificate
{
    ...
    public function __construct(
        ...
        $this->x509 = openssl_x509_parse(PemFormatter::addPemHeaders($valueInBase64));
    }

    // Subject's given name in title case
    public function getSubjectGN(): string
    {
        return mb_convert_case(mb_strtolower($this->x509['subject']['GN']), MB_CASE_TITLE);
    }

    // Subject's surname in title case
    public function getSubjectSN(): string
    {
        return mb_convert_case(mb_strtolower($this->x509['subject']['SN']), MB_CASE_TITLE);
    }
    ...
    {
        return SemanticsIdentifier::fromString($this->x509['subject']['serialNumber']);
    }
